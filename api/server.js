...
- **لیست بازی‌ها:**
  ...
  - **حدس کلمه (Hangman):** یک بازی ویژوال و پیشرفته برای حدس زدن اصطلاحات فنی.
...
**۳. فروشگاه جوایز:**
- کاربر با ارسال کلماتی مثل "فروشگاه" یا "جایزه" می‌تواند وارد فروشگاه شود.
- در فروشگاه می‌تواند با خرج کردن امتیاز، قفل محتوای آموزشی پیشرفته را باز کند.
- همچنین می‌تواند تم‌های ظاهری جدید برای چت‌بات خریداری کند.
`;
...
app.post("/api/getWelcomeMessage", async (req, res) => {
  ...
  const welcomePrompt = `
    شما یک دستیار هوشمند و دوستانه به نام 'پشتیبان گیمیفای‌شده' هستی.
    یک کاربر جدید به نام '${userName}' به تازگی وارد شده است.
    وظیفه شما این است که در بک پاراگراف ، یک پیام خوشامدگویی بسیار گرم و جذاب برای او بنویسی.
    ...
    4.  به بازی‌های تعاملی اشاره کن. بگو که با ارسال کلمه "بازی" می‌تواند شانسش را در بازی‌های متنوعی مثل "کوییز فنی"، "سنگ، کاغذ، قیچی"، "دوز" و بازی جدید و جذاب "حدس کلمه" امتحان کند.
    5.همجنین به او بگو که هر وقت خواست میتونه با نوشتن کلماتی مثل "فروشگاه" یا "جوایز" وارد فروشگاه محتوای ویژه بشه و با امتیازاتی که کسب کرده، محتوای ویژه یا تم بخرد.
